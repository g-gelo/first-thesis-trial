<template>
  <div class="chat-container relative min-h-screen">
    <div
      class="fixed inset-x-0 bottom-0 bg-white p-4 flex items-center z-50 border-t-2"
    >
      <input
        v-model="newMessage"
        class="flex-grow bg-slate-300 h-12 rounded-xl p-3 mr-2"
        type="text"
        placeholder="Message"
      />
      <button
        class="text-white p-2 rounded active:bg-slate-300"
        @click="newMessage.trim() !== '' ? sendMessage() : null"
      >
        <i class="fas fa-paper-plane text-green-800 text-2xl"></i>
      </button>
    </div>
    <div class="chat-content p-4 mb-20" fluid>
      <div>
        <div v-for="(message, index) in messages" :key="index">
          ...
          <div v-else>
            <div class="user">
              ...
              <div class="background card ma-2 pa-4 rounded-s-xl rounded-te-xl">
                <span class="message">{{ message.myText }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="showContactInfo" class="answer-container">
        ...
        <div v-else class="answer ma-2 pa-4 rounded-e-xl rounded-ts-xl">
          {{ messages[0].text3 }}
          <a href="https://www.facebook.com/CvSUImusAdmissions" target="_blank"
            >Click here</a
          >.
        </div>
      </div>
      ...
      <div
        ...
        class="start-message bg-gray-200 p-4 mb-4 rounded"
      >
        <span class="start-indicator text-gray-600"
          >Enter "start" to activate (Start conversation with Hi or Hello)</span
        >
      </div>
    </div>
  </div>
...
const sendMessage = () => {
  if (showStartMessage.value) {
    if (newMessage.value.toLowerCase().trim() === "start") {
      startConversation();
    } else {
      // Display a message informing the user to enter "start" to begin the conversation
      const userMessage = {
        me: "You",
  ...
  if (newMessage.value.trim() !== "") {
    ...
    for (let i = 0; i < keywords.length; i++) {
      ...
      if (newMessage.value.toLowerCase().includes(keyword.toLowerCase())) {
