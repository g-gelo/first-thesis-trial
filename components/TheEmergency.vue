<template>
  ...
  >
    <div
      class="flex items-start shadow-lg rounded-xl"
      v-if="emergency.isArchive == false"
    >
      ...
      <div class="w-60 p-4 border-l ml-2 border-green-800 relative">
          ...
          <span class="info-value main-title highlight"
            ...
            <div v-if="data?.user?.role == `SUPERADMIN`" class="relative">
              <button
                ...
                @click="
                  ($event) => {
                    editedEmergencyHotline.id = emergency.id;
                    editedEmergencyHotline.organization =
                      emergency.organization;
                    editedEmergencyHotline.number = emergency.number;
                    editedEmergencyHotline.location = emergency.location;
                    editedEmergencyHotline.isArchive = emergency.isArchive;
        ...
        <div class="mt-5 ma-2 seminar-info">
          <v-icon class="icon-small mr-3">fa-solid fa-location-dot</v-icon>
        ...
        <div class="mt-5 ma-2 seminar-info">
          ...
          <span class="info-value info-small pa-0">{{ emergency.number }}</span>
        </div>
      </div>
    </div>
  </div>
